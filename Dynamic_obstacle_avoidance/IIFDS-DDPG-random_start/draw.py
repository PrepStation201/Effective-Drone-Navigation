...
class Painter:
    def __init__(self, load_csv, load_dir=None):
        if not load_csv:
            ...
            if os.path.exists(self.load_dir):
                ...
                print("==The file under {} does not exist, Painter has automatically created the CSV".format(self.load_dir))
                self.data = pd.DataFrame(columns=['episode reward', 'episode', 'Method'])
        self.xlabel = None
        self.ylabel = None
        self.title = None
    ...
    def saveData(self, save_dir):
        ...
        print("==Data has been saved to the path {}!".format(save_dir))
    ...
    def smoothData(self, smooth_method_name,N):
        ...
        print("==Smoothing of data {} {} times completed!".format(smooth_method_name,N))
