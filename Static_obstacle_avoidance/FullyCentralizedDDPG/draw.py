...
class Painter:
    def __init__(self, load_csv, load_dir=None):
        if not load_csv:
            ...
            if os.path.exists(self.load_dir):
                print("Currently reading{}。".format(self.load_dir))
                self.data = pd.read_csv(self.load_dir).iloc[:,1:] 
                print("Read completed")
            else:
                print("==does not exist{}files under,Painter has automatically created the CSV file".format(self.load_dir))
                self.data = pd.DataFrame(columns=['episode reward', 'episode', 'Method'])
        self.xlabel = None
        self.ylabel = None
        self.title = None
    ...
    def addData(self, dataSeries, method, x=None, smooth = True):
        ...
        if x is not None:
            if len(x) != size:
                print("Please input x of the same dimension!")
                return
        for i in range(size):
            if x is not None:
            ...
            self.data = self.data.concat(dataToAppend,ignore_index = True)
    ...
    def saveData(self, save_dir):
        ...
        print("==The data has been saved to the path {}!".format(save_dir))
    ...
    def deleteData(self,delete_data_name):
        ...
        print("==Corresponding data under {} has been deleted!".format(delete_data_name))
    ...
    def smoothData(self, smooth_method_name,N):
        ...
        print("==Smoothing of {} data {} times is complete!".format(smooth_method_name,N))
