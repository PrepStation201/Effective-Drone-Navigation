...
class APF:
    def __init__(self):
        ...
        self.cone = Obstacle[env].cone              
        self.coneR = Obstacle[env].coneR            
        self.coneH = Obstacle[env].coneH            

        self.numberOfSphere = self.obstacle.shape[0]       
        self.numberOfCylinder = self.cylinder.shape[0]     
        self.numberOfCone = self.cone.shape[0]             
    ...
    def loop(self):             
        ...
        for i in range(500):
            qNext = self.getqNext(self.epsilon0,eta1List,eta2List,eta3List, q,qBefore)  
            qBefore = q
...
if __name__ == "__main__":
    ...
    apf.loop()
    apf.saveCSV()
    print('The trajectory distance is：',apf.calculateLength())
